...
import createUtilityPlugin from "./util/create-utility-plugin.js";
import escapeClassName from "./util/escape-class-name.js";
import flattenColorPalette from "./util/flatten-color-palette.js";
import isPlainObject from "./util/is-plain-object.js";
import log from "./util/log.js";
import { normalizeScreens } from "./util/normalize-screens.js";
import parseAnimationValue from "./util/parse-animation-value.js";
import { formatBoxShadowValue, parseBoxShadowValue } from "./util/parse-box-shadow-value.js";
import toColorValue from "./util/to-color-value.js";
import transformThemeValue from "./util/transform-theme-value.js";

const aileronVersion = pkg.version;
  ...
  pseudoElementVariants: ({ addVariant }) => {
    ...
    addVariant("before", ({ container }) => {
      container.walkRules((rule) => {
        ...
        if (!foundContent) {
          rule.prepend(postcss.decl({ prop: "content", value: "var(--adc-content)" }));
        }
      });
  ...
  darkVariants: ({ config, addVariant }) => {
    ...
    if (mode === false) {
      ...
      log.warn("darkmode-false", [
        "The `darkMode` option in your aileron CSS configuration is set to `false`, which now behaves the same as `media`.",
        "Change `darkMode` to `media` or remove it entirely.",
        "https://aileron.aa.com/"
      ]);
    }
...
const cssTransformValue = [
  "translate(var(--adc-translate-x), var(--adc-translate-y))",
  "rotate(var(--adc-rotate))",
  "skewX(var(--adc-skew-x))",
  "skewY(var(--adc-skew-y))",
  "scaleX(var(--adc-scale-x))",
  "scaleY(var(--adc-scale-y))"
].join(" ");
...
export const corePlugins = {
  preflight: ({ addBase }) => {
    ...
    addBase([
      postcss.comment({
        text: `! @aileron/styles v${aileronVersion} | MIT License | https://aileron.aa.com`
      }),
  ...
  borderSpacing: ({ addDefaults, matchUtilities, theme }) => {
    addDefaults("border-spacing", {
      "--adc-border-spacing-x": 0,
      "--adc-border-spacing-y": 0
    });
    ...
    matchUtilities(
      {
        "border-spacing": (value) => {
          return {
            "--adc-border-spacing-x": value,
            "--adc-border-spacing-y": value,
            "@defaults border-spacing": {},
            "border-spacing": "var(--adc-border-spacing-x) var(--adc-border-spacing-y)"
          };
        },
  ...
  translate: createUtilityPlugin(
    ...
    [
      [
        [
          "translate-x",
          [["@defaults transform", {}], "--adc-translate-x", ["transform", cssTransformValue]]
        ],
        [
          "translate-y",
          [["@defaults transform", {}], "--adc-translate-y", ["transform", cssTransformValue]]
        ]
      ]
    ],
    { supportsNegativeValues: true }
  ...
  rotate: createUtilityPlugin(
    "rotate",
    [["rotate", [["@defaults transform", {}], "--adc-rotate", ["transform", cssTransformValue]]]],
    { supportsNegativeValues: true }
  ),
  skew: createUtilityPlugin(
    ...
    [
      [
        ["skew-x", [["@defaults transform", {}], "--adc-skew-x", ["transform", cssTransformValue]]],
        ["skew-y", [["@defaults transform", {}], "--adc-skew-y", ["transform", cssTransformValue]]]
      ]
    ],
    { supportsNegativeValues: true }
  ),
  scale: createUtilityPlugin(
    ...
    [
      ...
      [
        [
          "scale-x",
          [["@defaults transform", {}], "--adc-scale-x", ["transform", cssTransformValue]]
        ],
        ["scale-y", [["@defaults transform", {}], "--adc-scale-y", ["transform", cssTransformValue]]]
      ]
    ],
    { supportsNegativeValues: true }
  ),
    ...
    addUtilities({
      ...
      ".transform-gpu": {
        transform: cssTransformValue.replace(
          "translate(var(--adc-translate-x), var(--adc-translate-y))",
          "translate3d(var(--adc-translate-x), var(--adc-translate-y), 0)"
        )
      },
  ...
  animation: ({ matchUtilities, theme, config }) => {
    const prefixName = (name) => `${config("prefix")}${escapeClassName(name)}`;
  ...
  touchAction: ({ addDefaults, addUtilities }) => {
    ...
    const cssTouchActionValue = "var(--adc-pan-x) var(--adc-pan-y) var(--adc-pinch-zoom)";
  ...
  scrollSnapType: ({ addDefaults, addUtilities }) => {
    addDefaults("scroll-snap-type", {
      "--adc-scroll-snap-strictness": "proximity"
    });
    ...
    addUtilities({
      ...
      ".snap-x": {
        ...
        "scroll-snap-type": "x var(--adc-scroll-snap-strictness)"
      },
      ".snap-y": {
        "@defaults scroll-snap-type": {},
        "scroll-snap-type": "y var(--adc-scroll-snap-strictness)"
      },
      ".snap-both": {
        "@defaults scroll-snap-type": {},
        "scroll-snap-type": "both var(--adc-scroll-snap-strictness)"
      },
      ".snap-mandatory": { "--adc-scroll-snap-strictness": "mandatory" },
      ".snap-proximity": { "--adc-scroll-snap-strictness": "proximity" }
    });
  },
  ...
  space: ({ matchUtilities, addUtilities, theme }) => {
        ...
        "space-x": (value) => {
          ...
          return {
            "& > :not([hidden]) ~ :not([hidden])": {
              "--adc-space-x-reverse": "0",
              "margin-right": `calc(${value} * var(--adc-space-x-reverse))`,
              "margin-left": `calc(${value} * calc(1 - var(--adc-space-x-reverse)))`
            }
          };
        },
        "space-y": (value) => {
          ...
          return {
            "& > :not([hidden]) ~ :not([hidden])": {
              "--adc-space-y-reverse": "0",
              "margin-top": `calc(${value} * calc(1 - var(--adc-space-y-reverse)))`,
              "margin-bottom": `calc(${value} * var(--adc-space-y-reverse))`
            }
          };
        }
      },
    ...
    addUtilities({
      ".space-y-reverse > :not([hidden]) ~ :not([hidden])": { "--adc-space-y-reverse": "1" },
      ".space-x-reverse > :not([hidden]) ~ :not([hidden])": { "--adc-space-x-reverse": "1" }
    });
  },
  ...
  divideWidth: ({ matchUtilities, addUtilities, theme }) => {
        ...
        "divide-x": (value) => {
          ...
          return {
            "& > :not([hidden]) ~ :not([hidden])": {
              ...
              "--adc-divide-x-reverse": "0",
              "border-right-width": `calc(${value} * var(--adc-divide-x-reverse))`,
              "border-left-width": `calc(${value} * calc(1 - var(--adc-divide-x-reverse)))`
            }
          };
        },
        "divide-y": (value) => {
          ...
          return {
            "& > :not([hidden]) ~ :not([hidden])": {
              "@defaults border-width": {},
              "--adc-divide-y-reverse": "0",
              "border-top-width": `calc(${value} * calc(1 - var(--adc-divide-y-reverse)))`,
              "border-bottom-width": `calc(${value} * var(--adc-divide-y-reverse))`
            }
          };
        }
      },
    ...
    addUtilities({
      ".divide-y-reverse > :not([hidden]) ~ :not([hidden])": {
        "@defaults border-width": {},
        "--adc-divide-y-reverse": "1"
      },
      ".divide-x-reverse > :not([hidden]) ~ :not([hidden])": {
        "@defaults border-width": {},
        "--adc-divide-x-reverse": "1"
      }
    });
  },
  ...
  divideColor: ({ matchUtilities, theme }) => {
    matchUtilities(
      {
        divide: (value) => ({
          "& > :not([hidden]) ~ :not([hidden])": {
  ...
  borderColor: ({ matchUtilities, theme, corePlugins }) => {
    ...
    matchUtilities(
      {
        "border-t": (value) => ({ "border-top-color": toColorValue(value) }),
        "border-r": (value) => ({ "border-right-color": toColorValue(value) }),
        "border-b": (value) => ({ "border-bottom-color": toColorValue(value) }),
        "border-l": (value) => ({ "border-left-color": toColorValue(value) })
      },
      {
        values: (({ DEFAULT: _, ...colors }) => colors)(flattenColorPalette(theme("borderColor"))),
        type: "color"
  ...
  backgroundColor: ({ matchUtilities, theme }) => {
    matchUtilities(
      {
        bg: (value) => ({"background-color": toColorValue(value)})
      },
  ...
  textColor: ({ matchUtilities, theme, corePlugins }) => {
    matchUtilities(
      { text: (value) => ({ color: toColorValue(value) }) },
  ...
  placeholderColor: ({ matchUtilities, theme }) => {
    matchUtilities(
      {
        placeholder: (value) => ({
          "&::placeholder": {
      ...
      matchUtilities(
        {
          shadow: (value) => {
            ...
            for (const shadow of ast) {
              ...
              shadow.color = "var(--adc-shadow-color)";
            }

            return {
              "@defaults box-shadow": {},
              "--adc-shadow": value === "none" ? "0 0 #0000" : value,
              "--adc-shadow-colored": value === "none" ? "0 0 #0000" : formatBoxShadowValue(ast),
    ...
    matchUtilities(
      {
        shadow: (value) => {
          return {
            "--adc-shadow-color": toColorValue(value),
            "--adc-shadow": "var(--adc-shadow-colored)"
          };
        }
      },
  ...
  content: createUtilityPlugin("content", [
    ["content", ["--adc-content", ["content", "var(--adc-content)"]]]
  ])
};
