...
export function normalizeConfig(config) {
  ...
  if (!valid) {
    log.warn("purge-deprecation", [
      "The `purge`/`content` options have changed in Aileron CSS v3.0.",
      "Update your configuration file to eliminate this warning.",
      "https://aileron.aa.com/docs/upgrade-guide#configure-content-sources"
    ]);
  }
  ...
  if (typeof config.prefix === "function") {
    log.warn("prefix-function", [
      "As of Aileron CSS v3.0, `prefix` cannot be a function.",
      "Update `prefix` in your configuration to be a string to eliminate this warning.",
      "https://aileron.aa.com/docs/upgrade-guide#prefix-cannot-be-a-function"
    ]);
    config.prefix = "";
  } else {
  ...
  for (const file of config.content.files) {
    if (typeof file === "string" && /{([^,]*?)}/g.test(file)) {
      log.warn("invalid-glob-braces", [
        `The glob pattern ${dim(file)} in your Aileron CSS configuration is invalid.`,
