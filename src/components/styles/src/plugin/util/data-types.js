...
import { parseBoxShadowValue } from "./parse-box-shadow-value.js";
...
const UNDERSCORE = /_(?![^(]*\))/g; // Underscore separator that is not located between brackets. E.g.: `rgba(255,_255,_255)_black` these don't count.
...
export function percentage(value) {
  return value.split(UNDERSCORE).every((part) => {
    return part.endsWith("%") || cssFunctions.some((fn) => new RegExp(`^${fn}\\(.+?%`).test(part));
  });
}
...
export function color(value) {
  ...
  const result = value.split(UNDERSCORE).every((part) => {
    part = normalize(part);
...
export function image(value) {
  let images = 0;
  const result = value.split(COMMA).every((part) => {
    part = normalize(part);
