...
import prefixSelector from "./prefix-selector.js";

const MERGE = ":merge";
const PARENT = "&";

export const selectorFunctions = new Set([MERGE]);
...
export function finalizeSelector(
  ...
  if (context?.aileronConfig?.prefix && !isArbitraryVariant) {
    format = prefixSelector(context.aileronConfig.prefix, format);
  }
...
function resolveFunctionArgument(haystack, needle, arg) {
  ...
  for (const char of haystack.slice(startIdx)) {
    if (char !== "(" && char !== ")") {
      target += char;
    } else if (char === "(") {
      target += char;
