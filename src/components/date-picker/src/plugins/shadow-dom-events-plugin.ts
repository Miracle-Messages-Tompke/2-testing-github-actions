import type { Handle } from "@adc/shared";
  ...
  const handleKeydown = (event: KeyboardEvent) => {
    ...
    if (key === "Enter") {
      target!.dispatchEvent(
        Object.assign(new CustomEvent("mousedown", { bubbles: true }), { which: 1 })
      );
      ...
      if (movedDateElem) {
        ...
        if (movedDate.getTime() < (innerDaysContainer.firstElementChild as any).dateObj.getTime()) {
          ...
          movedDate.getTime() > (innerDaysContainer.lastElementChild as any).dateObj.getTime()
        ) {
          fp.changeMonth(1);
